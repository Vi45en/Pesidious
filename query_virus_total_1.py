...
def evaluate(pefile):
    ...
    while(True):
        ...
        with open("mutated1.exe", 'wb') as file1:
            file1.write(bytez)

        score = virus_total_score("mutated1.exe")
        ...
        if(score < 40):
            with open("Mutated_malware/queried1_" + str(value) + ".exe", 'wb') as file1:
                file1.write(bytez)
            print("file wrote")
            value = value + 1
...
def imports_append(bytez):
    ...
    importsFile = open("imports1.txt" , 'w')
    ...
    with open("modified1.exe", 'wb') as file1:
        file1.write(bytez)

    sys.stdout.flush()
    cmd = " ./portable-executable/project-add-imports/bin/Debug/project-append-import modified1.exe imports1.txt modified1.exe"
    os.system(cmd)

    with open("modified1.exe", "rb") as binfile:
        bytez = binfile.read()
...
def section_add(bytez):    
    ...
    sys.stdout.flush()
    cmd = "./portable-executable/project-add-sections/bin/Debug/project-append-section modified1.exe " + section + " " + section_content + " modified1.exe"
    os.system(cmd)
