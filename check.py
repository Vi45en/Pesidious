...
def evaluate(pefile):
    ...
    #section renamed from .text to .orpc
    binary = lief.PE.parse(bytez, name="")
    targeted_section = binary.sections[0]
    targeted_section.name = ".orpc"
    bytez = __binary_to_bytez(binary)

    binary = lief.PE.parse(bytez, name="")
    targeted_section = binary.sections[1]
    targeted_section.name = "rqvmxkb"
    bytez = __binary_to_bytez(binary)

    binary = lief.PE.parse(bytez, name="")
    targeted_section = binary.sections[2]
    targeted_section.name = "rdata"
    bytez = __binary_to_bytez(binary)
    ...
    cmd = "./portable-executable/project-add-sections/bin/Debug/project-append-section" +  " modified.exe" + " camztlf " +  section_content + " modified.exe"
    os.system(cmd)

    #subprocess.call(["./portable-executable/project-add-sections/bin/Debug/project-append-section" , "modified.exe" , "camztlf" , section_content, "modified.exe"])
    ...
    cmd = "./portable-executable/project-add-sections/bin/Debug/project-append-section" +  " modified.exe" + " vryeypb " +  section_content + " modified.exe"
    os.system(cmd)
    ...
    with open("modified_0c7.exe", 'wb') as file1:
        file1.write(bytez)
